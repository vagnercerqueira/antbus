...
function renderMenuTop($items, $active = null, $i = 1, $id = null)
{
	...
	foreach ($items as $item) {
		...
		if (!empty($item['nodes'])) {
			...
			if ($item['id_pai'] == "" || $item['id_pai'] == 0) {
				$li = '<li class="nav-item dropdown" data-toggle="tooltip" title="' . $item['nome'] . '" data-placement="left">';
				$a =  '<a id="dropdownSubMenu' . ($i++) . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle" href="javascript:;" style="' . $cor . '"><i class="nav-icon ' . $icone . '"></i>&nbsp;</a>';
			} else {
